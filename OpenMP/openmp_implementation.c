...
void RSync_Process_Blocks(Matrix* matrix, Matrix* subMatrix, int id, int push) {
    ...
    for(r_mn = r_pq; r_mn < VIRTUALM; r_mn += PROCESSORS) {
        ...
        for(c_mn = c_pq; c_mn < VIRTUALN; c_mn += THREADS, step++){
            
            cbegin = c_mn * coffset;
            cend = cbegin + coffset;

            subr = (step / (VIRTUALM/PROCESSORS));
            subc = (step % (VIRTUALN/PROCESSORS));
...
void Write_ToFile(int sizex, int sizey, Matrix* matrix, char* filename) {
    int ix = 0;
    int iy = 0;
    FILE *fp;

    fp = fopen(filename, "w");
    for (iy = sizey-1; iy >= 0; iy--) {
        for (ix = 0; ix <= sizex-1; ix++) {
            fprintf(fp, "%6.1f", matrix->items[ix][iy].data);
            if (ix != sizex-1) fprintf(fp, " ");
            else fprintf(fp, "\n");
        }
    }
    fclose(fp);
...
int main(int argc, char*argv[]) {
    ...
    Write_ToFile(A_RN, A_CN, A, "init.dat");
                                       ...
                                       private(sa, sb, sc, ID, t, i, j, l, jp, ip, subsa, subsb, subsc) num_threads(PROCESSORS * THREADS)
    {
        ...
        for(t = 0; t < LCM; t++) {
            ...
            for(i = 0; i < (VIRTUALM / PROCESSORS); i++){
                for(j = 0; j < (VIRTUALN / THREADS); j++){
                    for(l = 0; l < (VIRTUALK / LCM); l++){
                        ...
                        RSync_Process_SubMatrix(sc, subsc, i, j, 1);
                    }
                }
            }
            ...
            #pragma omp single
            {
                LeftShiftMatrix(A, BLOCK_SIZE, 0);
                LeftShiftMatrix(B, BLOCK_SIZE, 0);
            }
        }
    }

    Write_ToFile(A_RN, B_CN, C, "final.dat");
